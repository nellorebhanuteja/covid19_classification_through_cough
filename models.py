...
class SVM(sklearnModel):
	def __init__(self, model_args): 
		...
		self.classifier = SVC(C=int(model_args['C']), max_iter=int(model_args['max_iter']),decision_function_shape=model_args['decision_function_shape'],kernel=model_args['kernel'],verbose=model_args['verbose'],gamma=model_args['gamma'],random_state=model_args['random_state'],probability=True, tol=float(model_args['tol']))