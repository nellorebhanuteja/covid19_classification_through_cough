...
def main(config,datadir,outdir):
    ...
    pca.fit(train_feats)
    print('PCA training ended')
    train_feats = pca.transform(train_feats)
    pk.dump(pca, open(
        outdir + '/' + config['default']['pca_model'] + '.pkl',"wb"))
    ...
    if model_type == 'LogisticRegression':
        ...
        model_args = {'C':int(config[model_type]['C']),
                        'max_iter':int(config[model_type]['max_iter']),
                        'gamma':config[model_type]['gamma'],
                        'decision_function_shape':config[model_type]['decision_function_shape'],
                        'kernel':config[model_type]['kernel'],
                        'verbose': verbose,
                        'probability':True,
                        'tol':float(config[model_type]['tol']),
                        'random_state':int(config['default']['seed'])}
