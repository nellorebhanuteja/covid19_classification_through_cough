...
import librosa
import numpy as np
from feature_extraction_jio import feature_extraction_jio, iitg_features, cambridge_features
...
def main(config, in_wav_list, out_folder):
	...
	feats_list=[]
	pbar = trange(len(in_wav_list), leave=None)
	for i in pbar:
		
		file_id = in_wav_list[i][0]
		file_path = in_wav_list[i][1]
		...
		if len(s)<int(config['default']['window_size']):
			continue

		if config['default']['feature_type'] == 'mfcc':
			f = compute_mfcc(s,config)
		elif config['default']['feature_type'] == 'jio_feature':
			frames = librosa.util.frame(
				x=s,
				frame_length=int(config['default']['window_size']),
				hop_length=int(config['default']['window_shift'])
			)
			
			f = []
			count=1
			for frame in frames.transpose():
				feature_frame = feature_extraction_jio(
					frame,
					int(config['default']['window_size']),
					int(config['default']['window_shift']),
					fs,
					int(config['default']['window_size'])
				)
				if count == 1:
					f = feature_frame.flatten()
				else:
					f = np.column_stack((f, feature_frame.flatten()))
				count+=1
			# f_delta = librosa.feature.delta(f, width=9, order=2, axis=-1)
			# f_delta_delta = librosa.feature.delta(f, width=3, order=2, axis=-1)
			# f = np.row_stack(
			# 	[
			# 		f, f_delta, f_delta_delta
			# 	]
			# )
		else:
